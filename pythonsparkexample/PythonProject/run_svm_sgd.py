...
@LastEditTime: 2020-07-29 08:13:53
...
def parametersEval(trainData, validationData):
    ...
    evalParameter(trainData, validationData,"iterations", 
                              iterations_list=[1, 5, 15, 25, 50, 100], 
                              step_list=[1.0],
                              regParam_list=[0.01])

    print("----- 评估step参数使用 ---------")  
    evalParameter(trainData, validationData, "step", 
                              iterations_list=[25], 
                              step_list=[0.1, 1.0, 5.0, 10.0, 50.0],
                              regParam_list=[0.01])
    ...
    evalParameter(trainData, validationData,"regParam", 
                              iterations_list=[25],
                              step_list=[1.0],                
                              regParam_list=[0.001, 0.01, 0.1, 0., 1.])   


def evalParameter(trainData, validationData, evalparm,
                  iterations_list, step_list, regParam_list):
    #训练评估参数
    metrics = [train_evaluate_model(trainData, validationData,  
                                iterations,step, regParam) 
    ...
    df = pd.DataFrame(metrics,index=IndexList,
            columns=['AUC', 'duration','iterations', 'step', 'regParam', 'model'])
    #显示图形
...
def eval_all_parameter(train_data, validation_data, iterations_list, step_list, regParam_list):
    metrics = [train_evaluate_model(trainData, validationData,  
                                iterations, step, regParam) 
                       for iterations in iterations_list
                       for step in step_list
    ...
    print("调校后最佳参数：iterations:{}, step: {}, regParam: {}\n AUC={}".format(
...
if __name__ == "__main__":
    ...
    AUC, duration, *_, model = \
        train_evaluate_model(trainData, validationData, 50, 1.0, 0.01)
