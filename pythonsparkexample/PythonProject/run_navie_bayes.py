...
@LastEditTime: 2020-07-28 21:36:54
...
def convert(x):
    ...
    return 0 if ret < 0 else ret  # 特征字段大于0
...
def get_features(fields, categories_map, feature_end):
    ...
    #提取数值字段 后面的字段都是数值类型
    numericalFeatures=[convert(field) for field in fields[4: feature_end]] 
    #返回“分类特征字段”+“数值特征字段”
...
def prepare_data(sc):
    ...
    for x in features_rdd.first():
        print(x, end=', ')
    print("")
    # NB withMean=False
    scaler = StandardScaler(withMean=False, withStd=True).fit(features_rdd)
    stand_features = scaler.transform(features_rdd)
    print('标准化之后: \n') 
    for x in stand_features.first():
        print(x, end=', ')
    print("")
...
def evaluate_model(model, valid_data):
    ...
    # 
    score_and_labels = score.map(lambda x: x).\
        zip(valid_data.map(lambda p: p.label)).\
...
def parametersEval(trainData, validationData):
    ...
    evalParameter(trainData, validationData,"lambda_", 
                              lambda_list=[1.0, 3.0, 5.0, 10.0, 15.0, 20.0,
                                25.0, 30., 35., 40., 45., 50., 60.], 
                              ) 


def evalParameter(trainData, validationData, evalparm, lambda_list):
    #训练评估参数
    metrics = [train_evaluate_model(trainData, validationData, lambda_) 
                for lambda_ in lambda_list]
    #设置当前评估的参数
    if evalparm=="lambda_":
        IndexList=lambda_list[:]

    #转换为Pandas DataFrame
    df = pd.DataFrame(metrics,index=IndexList,
            columns=['AUC', 'duration','lambda_', 'model'])
    #显示图形
...
def eval_all_parameter(train_data, validation_data, lambda_list):
    metrics = [train_evaluate_model(trainData, validationData, lambda_) 
                for lambda_ in lambda_list]
    # 根据AUC 选择最佳
    best = sorted(metrics, key=lambda k: k[0], reverse=True)[0]
    print("调校后最佳参数：lambda:{} \n AUC={}".format(
        best[2], best[0]))
...
if __name__ == "__main__":
    ...
    testData.persist()
    print("==========训练评估阶段===============")
    AUC, duration, *_, model = \
        train_evaluate_model(trainData, validationData, 30.)
    ...
    testData.unpersist()
    save_model(model, sc, 'Navie_Bayes')
